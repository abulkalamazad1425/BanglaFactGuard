import { Component, OnInit, inject, signal } from '@angular/core';
import { CommonModule } from '@angular/common';
import { RouterLink } from '@angular/router';
import { ApiService } from '../../../core/services/api.service';
import { SubmissionSummary, SubmissionStats } from '../../../core/models/verification.model';
import { VerdictBadgeComponent } from '../../../shared/components/verdict-badge/verdict-badge.component';
import { API_ENDPOINTS } from '../../../core/constants/api-endpoints.constant';

@Component({
  selector: 'app-submission-history',
  standalone: true,
  imports: [CommonModule, RouterLink, VerdictBadgeComponent],
  template: `
    <div class="history-page">
      <div class="container" style="padding-top:48px; padding-bottom:80px;">
        <div class="page-header animate-in">
          <h1>My Submissions</h1>
          <p>Your fact-checking history</p>
        </div>

        @if (stats()) {
          <div class="stats-grid animate-in">
            <div class="stat-card card">
              <span class="stat-num">{{ stats()!.total }}</span>
              <span class="stat-lbl">Total</span>
            </div>
            <div class="stat-card card text-success">
              <span class="stat-num">{{ stats()!.finalized_true }}</span>
              <span class="stat-lbl">True</span>
            </div>
            <div class="stat-card card text-error">
              <span class="stat-num">{{ stats()!.finalized_false }}</span>
              <span class="stat-lbl">False</span>
            </div>
            <div class="stat-card card text-warning">
              <span class="stat-num">{{ stats()!.finalized_partially_true }}</span>
              <span class="stat-lbl">Partially True</span>
            </div>
            <div class="stat-card card">
              <span class="stat-num">{{ stats()!.pending }}</span>
              <span class="stat-lbl">Pending</span>
            </div>
          </div>
        }

        @if (loading()) { <div class="loading-overlay"><div class="spinner"></div></div> }

        @if (!loading() && submissions().length === 0) {
          <div class="empty-state card text-center">
            <div style="font-size:64px; margin-bottom:16px;">📭</div>
            <h3>No submissions yet</h3>
            <p>Start verifying news claims to see them here.</p>
            <a routerLink="/verify" class="btn btn-primary mt-4">Verify a Claim</a>
          </div>
        }

        @if (!loading() && submissions().length > 0) {
          <div class="table-card card animate-in">
            <table class="data-table">
              <thead>
                <tr>
                  <th>Headline</th>
                  <th>Source</th>
                  <th>Verdict</th>
                  <th>Confidence</th>
                  <th>Submitted</th>
                  <th></th>
                </tr>
              </thead>
              <tbody>
                @for (s of submissions(); track s.claim_id) {
                  <tr>
                    <td class="headline-cell">{{ s.headline | slice:0:80 }}{{ s.headline.length > 80 ? '...' : '' }}</td>
                    <td class="source-cell text-muted text-sm">{{ s.claimed_source | slice:0:40 }}</td>
                    <td><app-verdict-badge [label]="s.ai_label" /></td>
                    <td>
                      @if (s.ai_confidence !== null) {
                        <span class="confidence">{{ (s.ai_confidence * 100).toFixed(1) }}%</span>
                      } @else { <span class="text-muted">—</span> }
                    </td>
                    <td class="text-muted text-sm">{{ s.submitted_at | date:'MMM d, y' }}</td>
                    <td><a [routerLink]="['/verify', s.claim_id]" class="btn btn-ghost btn-sm">View</a></td>
                  </tr>
                }
              </tbody>
            </table>
          </div>

          <div class="pagination">
            <button class="btn btn-ghost btn-sm" [disabled]="offset() === 0" (click)="prev()">← Prev</button>
            <span class="text-muted text-sm">Page {{ page() }}</span>
            <button class="btn btn-ghost btn-sm" [disabled]="submissions().length < limit" (click)="next()">Next →</button>
          </div>
        }
      </div>
    </div>
  `,
  styles: [`
    .history-page { background: var(--bg-base); min-height: 100%; }
    .page-header { margin-bottom: 32px; h1 { margin-bottom: 8px; } p { color: var(--text-secondary); } }
    .stats-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 16px; margin-bottom: 32px;
      @media (max-width: 768px) { grid-template-columns: repeat(3, 1fr); }
      @media (max-width: 480px) { grid-template-columns: repeat(2, 1fr); }
    }
    .stat-card { text-align: center; padding: 20px 16px; }
    .stat-num { display: block; font-size: 2rem; font-weight: 800; font-family: var(--font-display); }
    .stat-lbl { font-size: 12px; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.05em; }
    .table-card { padding: 0; overflow: hidden; margin-bottom: 24px; }
    .headline-cell { max-width: 320px; }
    .source-cell { max-width: 200px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    .confidence { font-weight: 600; color: var(--primary-light); }
    .empty-state { max-width: 400px; margin: 80px auto; }
    .pagination { display: flex; align-items: center; justify-content: center; gap: 16px; }
    .loading-overlay { display: flex; justify-content: center; padding: 80px; }
  `]
})
export class SubmissionHistoryComponent implements OnInit {
  private readonly api = inject(ApiService);

  readonly loading = signal(true);
  readonly submissions = signal<SubmissionSummary[]>([]);
  readonly stats = signal<SubmissionStats | null>(null);
  readonly offset = signal(0);
  readonly limit = 20;

  readonly page = () => Math.floor(this.offset() / this.limit) + 1;

  ngOnInit(): void {
    this.api.get<SubmissionStats>(API_ENDPOINTS.USERS_SUBMISSION_STATS).subscribe({
      next: s => this.stats.set(s), error: () => {}
    });
    this.load();
  }

  load(): void {
    this.loading.set(true);
    this.api.get<SubmissionSummary[]>(API_ENDPOINTS.USERS_SUBMISSIONS, { limit: this.limit, offset: this.offset() })
      .subscribe({ next: s => { this.submissions.set(s); this.loading.set(false); }, error: () => this.loading.set(false) });
  }

  prev(): void { this.offset.update(o => Math.max(0, o - this.limit)); this.load(); }
  next(): void { this.offset.update(o => o + this.limit); this.load(); }
}
